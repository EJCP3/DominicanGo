...
const getApiUrl = () => {
  const raw = "https://dominicango-api.onrender.com/api";
  if (raw.trim()) {
...
const getProvincesWithDestinations = async (params = {}) => {
...
const getFavoriteIds = async (token) => {
  const empty = { destinationIds: /* @__PURE__ */ new Set(), blogIds: /* @__PURE__ */ new Set() };
  ...
  try {
    ...
    const destinationIds = new Set(
      favorites.filter((f) => f.type === "DESTINATION" && f.destinationId).map((f) => f.destinationId)
    );
    const blogIds = new Set(
      favorites.filter((f) => f.type === "BLOG" && f.blogId).map((f) => f.blogId)
    );
...
export { getApiUrl as a, getProvincesWithDestinations as b, getBlogs as c, getFavoriteIds as d, baseProvinces as e, getDestinations as f, getUser as g, slugify as s };
